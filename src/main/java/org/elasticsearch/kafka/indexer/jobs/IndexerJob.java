...
public class IndexerJob implements Callable<IndexerJobStatus> {
	...
	void initKafka() throws Exception {
		...
		if (consumerGroupName.isEmpty()) {
			...
			logger.info("ConsumerGroupName was empty, set it to {} for partition {}", consumerGroupName,
					currentPartition);
		}
	...
	private void initElasticSearch() throws Exception {
		...
		try {
			Settings settings = ImmutableSettings.settingsBuilder().put("cluster.name", consumerConfig.esClusterName)
					.build();
			...
			for (String eachHostPort : esHostPortList) {
				...
				esClient.addTransportAddress(new InetSocketTransportAddress(eachHostPort.split(":")[0].trim(),
			...
			logger.error(
					"Exception when trying to connect and create ElasticSearch Client. Throwing the error. Error Message is::"
							+ e.getMessage());
			throw e;
		}
	}
	...
	void reInitKafka() throws Exception {
		for (int i = 0; i < numberOfReinitAttempts; i++) {
			try {
				logger.info("Re-initializing Kafka for partition {}, try # {}", currentPartition, i);
				...
				logger.info("Kafka Reintialization  for partition {} finished OK", currentPartition);
				return;
			} catch (Exception e) {
				if (i < numberOfReinitAttempts) {
					...
					// if we got here - we failed to re-init Kafka after
					// numberOfTries attempts - throw an exception out
					logger.info(
							"Kafka Re-initialization failed for partition {} after {} attempts - throwing exception: "
									+ e.getMessage(),
							currentPartition, numberOfReinitAttempts);
					throw e;
				}
			}
		}
	...
	private void createMessageHandler() throws Exception {
		try {
			logger.info("MessageHandler Class given in config is {} for partition {}",
					consumerConfig.messageHandlerClass, currentPartition);
			msgHandler = (MessageHandler) Class.forName(consumerConfig.messageHandlerClass)
					.getConstructor(TransportClient.class, ConsumerConfig.class).newInstance(esClient, consumerConfig);
			logger.debug("Created and initialized MessageHandler: {} for partition {}",
					consumerConfig.messageHandlerClass, currentPartition);
		} catch (Exception e) {
	...
	// a hook to be used by the Manager app to request a graceful shutdown of
	// the job
	...
	private void computeOffset() throws Exception {
		...
		if (consumerConfig.startOffsetFrom.equalsIgnoreCase("CUSTOM")) {
			if (consumerConfig.startOffset >= 0) {
				...
				throw new Exception("Custom start offset for topic [" + currentTopic + "], partition ["
						+ currentPartition
						+ "] is < 0, which is not an acceptable value - please provide a valid offset; exiting");
			}
			...
			logger.info("Restarting from where the Offset is left for topic {}, for partition {}", currentTopic,
					currentPartition);
			offsetForThisRound = kafkaConsumerClient.fetchCurrentOffsetFromKafka();
			if (offsetForThisRound == -1) {
				// if this is the first time this client tried to read - offset
				// might be -1
				// [ TODO figure out all cases when this could happen]
				// try to get the Earliest offset and read from there - it may
				// lead
				// to processing events that may have already be processed - but
				// it is safer than
				// starting from the Latest offset in case not all events were
				// processed before
				offsetForThisRound = kafkaConsumerClient.getEarliestOffset();
				logger.info(
						"offsetForThisRound is set to the EarliestOffset since currentOffset is -1; offsetForThisRound={} for partition {}",
						offsetForThisRound, currentPartition);
				// also store this as the CurrentOffset to Kafka - to avoid the
				// multiple cycles through
				// this logic in the case no events are coming to the topic for
				// a long time and
				// we always get currentOffset as -1 from Kafka
				try {
					kafkaConsumerClient.saveOffsetInKafka(offsetForThisRound, ErrorMapping.NoError());
				} catch (Exception e) {
					logger.error("Failed to commit the offset in Kafka, exiting for partition {}: ", currentPartition,
							e);
					throw new Exception("Failed to commit the offset in Kafka, exiting: " + e.getMessage(), e);
				}

			} else {
				logger.info("offsetForThisRound is set to the CurrentOffset: {} for partition {}", offsetForThisRound,
						currentPartition);
			}
		}
		long earliestOffset = kafkaConsumerClient.getEarliestOffset();
		logger.info("EarliestOffset for partition {} is {}", currentPartition, earliestOffset);
		// check for a corner case when the computed offset (either current or
		// custom)
		// is less than the Earliest offset - which could happen if some
		// messages were
		// cleaned up from the topic/partition due to retention policy
		if (offsetForThisRound < earliestOffset) {
			logger.warn(
					"WARNING: computed offset (either current or custom) = {} is less than EarliestOffset = {}"
							+ "; setting offsetForThisRound to the EarliestOffset for partition {}",
					offsetForThisRound, earliestOffset, currentPartition);
	...
	public IndexerJobStatus call() {
		...
		while (!shutdownRequested) {
			try {
				// check if there was a request to stop this thread - stop
				// processing if so
				...
				logger.debug("******* Starting a new batch of events from Kafka for partition {} ...",
						currentPartition);
				...
				logger.error(
						"Exception when starting a new round of kafka Indexer job for partition {} - will try to re-init Kafka ",
						currentPartition, e);
				// try to re-init Kafka connection first - in case the leader
				// for this partition
				// has changed due to a Kafka node restart and/or leader
				// re-election
				try {
					this.reInitKafka();
				} catch (Exception e2) {
					// we still failed - do not keep going anymore - stop and
					// fix the issue manually,
					// then restart the consumer again; It is better to monitor
					// the job externally
					// via Zabbix or the likes - rather then keep failing
					// [potentially] forever
	...
	public void processBatch() throws Exception {
		...
		if (!isStartingFirstTime) {
			// do not read offset from Kafka after each run - we just stored it
			// there
			// If this is the only thread that is processing data from this
			// partition -
			...
			try {
				...
				logger.error("Exception getting Kafka offsets for partition {}, will try to re-init Kafka ",
						currentPartition, e);
				reInitKafka();
				// if re-initialization was successful - return and let the next
				// job run try to
				// pickup from where it was left of before
				// if it failed - an exception will be thrown out of the
				// reInitKafka()
				return;
			}
			// mark this as not first time startup anymore - since we already
			// saved correct offset
			// to Kafka, and to avoid going through the logic of figuring out
			// the initial offset
			// every round if it so happens that there were no events from Kafka
			// for a long time
		...
		if (fetchResponse.hasError()) {
			...
			// TODO figure out what job status we should use - PartialFailure??
			// and when to clean it up?
			handleError();
			return;
		}
		...
		if (byteBufferMsgSet.validBytes() <= 0) {
			logger.debug("No events were read from Kafka - finishing this round of reads from Kafka for partition {}",
					currentPartition);
			// TODO re-review this logic
			long latestOffset = kafkaConsumerClient.getLastestOffset();
			if (latestOffset != offsetForThisRound) {
				logger.warn(
						"latestOffset [{}] is not the same as the current offsetForThisRound for this run [{}]"
								+ " - committing latestOffset to Kafka for partition {}",
						latestOffset, offsetForThisRound, currentPartition);
				try {
					kafkaConsumerClient.saveOffsetInKafka(latestOffset,
							fetchResponse.errorCode(consumerConfig.topic, currentPartition));
				} catch (Exception e) {
					// throw an exception as this will break reading messages in
					// the next round
					logger.error("Failed to commit the offset in Kafka - exiting for partition {} ", currentPartition,
							e);
					throw e;
		...
		if (isDryRun) {
			logger.info(
					"**** This is a dry run, NOT committing the offset in Kafka nor posting to ES for partition {}****",
					currentPartition);
			return;
		}
		...
		try {
			...
			//return and re-process current batch
			return;
		}
		if (consumerConfig.isPerfReportingEnabled) {
			long timeAftEsPost = System.currentTimeMillis();
			logger.debug("Approx time to post of ElasticSearch: {} ms for partition {}", (timeAftEsPost - jobStartTime),
					currentPartition);
		}
		logger.info("Commiting offset: {} for partition {}", nextOffsetToProcess, currentPartition);
		// TODO optimize getting of the fetchResponse.errorCode - in some cases
		// there is no error,
		// so no need to call the API every time
		try {
			kafkaConsumerClient.saveOffsetInKafka(nextOffsetToProcess,
					fetchResponse.errorCode(consumerConfig.topic, currentPartition));
		} catch (Exception e) {
			logger.error("Failed to commit the Offset in Kafka after processing and posting to ES for partition {}: ",
					currentPartition, e);
			logger.info("Trying to reInitialize Kafka and commit the offset again for partition {}...",
					currentPartition);
			try {
				reInitKafka();
				...
				logger.error(
						"Failed to commit the Offset in Kafka even after reInitializing Kafka - exiting for partition {}: ",
						currentPartition, e2);
				// there is no point in continuing - as we will keep
		...
		logger.info(
				"*** Finished current round of ConsumerJob, processed messages with offsets [{}-{}] for partition {} ****",
	...
	private void reInitElasticSearch() throws InterruptedException, IndexerESException {
		for (int i = 1; i <= numberOfEsIndexingRetryAttempts; i++) {
			...
			logger.warn("Retrying connect to ES and re-process batch, partition {}, try# {}", currentPartition, i);
			try {
				...
				if (i < numberOfEsIndexingRetryAttempts) {
					...
					// we've exhausted the number of retries - throw a
					// IndexerESException to stop the IndexerJob thread
					logger.error(
							"Retrying connect to ES and re-process batch after connection failure, partition {}, "
									+ "try# {} - failed after the last retry; Will keep retrying, ",
							currentPartition, i);

					indexerJobStatus.setJobStatus(IndexerJobStatusEnum.Hanging);
					throw new IndexerESException(
							"Indexing into ES failed due to connectivity issue to ES, partition: " + currentPartition);
				}
			}
		}
	}
	...
	private void indexIntoESWithRetries() throws IndexerESException, Exception {
		try {
			...
			// ES cluster is unreachable or down. Re-try up to the configured
			// number of times
			// if fails even after then - shutdown the current IndexerJob
			logger.error(
					"Error posting messages to Elastic Search for offset {}-->{} "
			...
			// but be aware that ALL messages from this batch are NOT indexed
			// into ES
			...
			FailedEventsLogger.logFailedEvent(offsetForThisRound, nextOffsetToProcess - 1, currentPartition,
					e.getDetailedMessage(), null);
		}
	...
	public void handleError() throws Exception {
		...
		logger.error("Error fetching events from Kafka - handling it. Error code: {}  for partition {}", errorCode,
				currentPartition);
		if (errorCode == ErrorMapping.BrokerNotAvailableCode()) {
			logger.error(
					"BrokerNotAvailableCode error happened when fetching message from Kafka. ReInitiating Kafka Client for partition {}",
					currentPartition);
			reInitKafka();
		} else if (errorCode == ErrorMapping.InvalidFetchSizeCode()) {
			logger.error(
					"InvalidFetchSizeCode error happened when fetching message from Kafka. ReInitiating Kafka Client for partition {}",
					currentPartition);
			reInitKafka();
		} else if (errorCode == ErrorMapping.InvalidMessageCode()) {
			logger.error(
					"InvalidMessageCode error happened when fetching message from Kafka, not handling it. Returning for partition {}",
					currentPartition);
		} else if (errorCode == ErrorMapping.LeaderNotAvailableCode()) {
			logger.error(
					"LeaderNotAvailableCode error happened when fetching message from Kafka. ReInitiating Kafka Client for partition {}",
					currentPartition);
			reInitKafka();
		} else if (errorCode == ErrorMapping.MessageSizeTooLargeCode()) {
			logger.error(
					"MessageSizeTooLargeCode error happened when fetching message from Kafka, not handling it. Returning for partition {}",
					currentPartition);
		} else if (errorCode == ErrorMapping.NotLeaderForPartitionCode()) {
			logger.error(
					"NotLeaderForPartitionCode error happened when fetching message from Kafka, not handling it. ReInitiating Kafka Client for partition {}",
					currentPartition);
			reInitKafka();
		} else if (errorCode == ErrorMapping.OffsetMetadataTooLargeCode()) {
			logger.error(
					"OffsetMetadataTooLargeCode error happened when fetching message from Kafka, not handling it. Returning for partition {}",
					currentPartition);
			...
			long earliestOffset = kafkaConsumerClient.getEarliestOffset();
			// The most likely reason for this error is that the consumer is
			// trying to read events from an offset
			// that has already expired from the Kafka topic due to retention
			// period;
			// In that case the only course of action is to start processing
			// events from the EARLIEST available offset
			logger.info(
					"OffsetOutOfRangeCode error: setting offset for partition {} to the EARLIEST possible offset: {}",
			...
			try {
				...
				// throw an exception as this will break reading messages in the
				// next round
				// TODO verify that the IndexerJob is stopped cleanly in this
				// case
				logger.error("Failed to commit offset in Kafka after OffsetOutOfRangeCode - exiting for partition {} ",
						currentPartition, e);
				throw e;
			}
			return;
			...
			logger.error(
					"ReplicaNotAvailableCode error happened when fetching message from Kafka - re-init-ing Kafka... for partition {}",
					currentPartition);
			reInitKafka();
			return;
			...
			logger.error(
					"RequestTimedOutCode error happened when fetching message from Kafka - re-init-ing Kafka... for partition {}",
					currentPartition);
			reInitKafka();
			return;
			...
			logger.error(
					"StaleControllerEpochCode error happened when fetching message from Kafka, not handling it. Returning for partition {}",
					currentPartition);
			return;

		} else if (errorCode == ErrorMapping.StaleLeaderEpochCode()) {
			logger.error(
					"StaleLeaderEpochCode error happened when fetching message from Kafka, not handling it. Returning for partition {}",
					currentPartition);
			return;

		} else if (errorCode == ErrorMapping.UnknownCode()) {
			logger.error(
					"UnknownCode error happened when fetching message from Kafka - re-init-ing Kafka... for partition {}",
					currentPartition);
			reInitKafka();
			return;
			...
			logger.error(
					"UnknownTopicOrPartitionCode error happened when fetching message from Kafka - re-init-ing Kafka...for partition {}",
					currentPartition);
			reInitKafka();
			return;
	...
	public void stopClients() {
		logger.info("About to stop ES client for topic {}, partition {}", currentTopic, currentPartition);
		if (esClient != null)
			esClient.close();
		logger.info("About to stop Kafka client for topic {}, partition {}", currentTopic, currentPartition);
		if (kafkaConsumerClient != null)
			kafkaConsumerClient.close();
		logger.info("Stopped Kafka and ES clients for topic {}, partition {}", currentTopic, currentPartition);
	}
